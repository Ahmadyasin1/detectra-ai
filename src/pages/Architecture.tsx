...
export default function Architecture() {
  return (
    <div className="min-h-screen bg-transparent">
      <PageHero
        badge="Production Architecture · 6-Layer Design"
        badgeIcon={Network}
        title="System"
        titleAccent="Architecture"
        description="A 6-layer production architecture with clear boundaries between ingestion, processing, intelligence, reasoning, storage, and presentation — all communicating via async message queues."
      />
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {LAYERS.map((layer, i) => (
              <motion.div
                ...
                className={`card-glass p-5 border ${layer.border} relative overflow-hidden group hover:scale-[1.02] transition-transform duration-200`}
        ...
        <section>
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="card-glass p-5 border-purple-500/20"
            style={{ boxShadow: '0 0 20px rgba(168,85,247,0.08)' }}
          >
            <div className="flex flex-wrap items-center gap-0">
              {FLOW.map((node, i) => (
                <div key={node} className="flex items-center gap-1">
                  <span className="px-2.5 py-1.5 bg-white/10 border border-white/20 rounded-lg text-xs text-white font-mono whitespace-nowrap hover:border-cyan-500/50 transition-colors">
                    {node}
                  </span>
                  {i < FLOW.length - 1 && (
                ...
                <tbody>
                  {COMPONENTS.map((c, i) => (
                    <motion.tr
                      ...
                      className="border-b border-white/10 last:border-0 hover:bg-white/20 transition-colors"
                    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {AGENTS.map((agent, i) => (
              <motion.div
                ...
                className={`card-glass p-5 border ${agent.border} ${agent.bg}`}
              >
                <div className="mb-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <motion.div
                ...
                className={`card-glass p-5 border ${mode.border}`}
              >
