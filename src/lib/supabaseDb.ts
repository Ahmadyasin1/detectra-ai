...
const STORAGE_BUCKET = import.meta.env.VITE_SUPABASE_STORAGE_BUCKET || 'video_uploads';
...
export async function uploadVideoFileToBucket(
  ...
  if (publicUrlError) {
    logSupabaseError('getPublicUrl', publicUrlError);
  }

  return {
    storagePath,
    publicUrl: publicUrlData?.publicUrl ?? null,
    error: publicUrlError?.message ?? null,
  };
}
